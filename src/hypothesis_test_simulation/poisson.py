...
from src.hypothesis_test.poisson import poisson_diff_test
from src.sample_size.poisson import poisson_diff_test_sample_size
...
def main():
    ...
    for test_case in test_cases:
        ...
        if test_case["alternative"] == "two-sided":
            nA, nB = poisson_diff_test_sample_size(
                lambdaA,
                test_case["lambdaB"],
            ...
            nA, nB = poisson_diff_test_sample_size(
                lambdaA, test_case["lambdaB"], "one-sided", alpha, power, alloc_ratio
            )
        else:
        ...
        for i in tqdm(range(n_sim)):
            ...
            # 同じ分布からデータを生成
            occurrenceA = np.sum(rng.poisson(lambdaA, nA))
            occurrenceB = np.sum(rng.poisson(lambdaA, nB))

            _, p = poisson_diff_test(
        ...
        for i in tqdm(range(n_sim)):
            ...
            # 異なる分布からデータを生成
            occurrenceA = np.sum(rng.poisson(lambdaA, nA))
            occurrenceB = np.sum(rng.poisson(test_case["lambdaB"], nB))
    ...
    output_path = result_dir.joinpath("poisson.json")
